...
const TEACHER_UUID = process.env.TEACHER_UUID; // Ensure this is set in .env.local
...
async function insertQuestions() {
    const filePath = path.join(__dirname, '../docs/SCIENCE_QUESTIONS.json');
    ...
    for (const q of questions) {
        ...
        const { data: qData, error: qError } = await supabase
            ...
            .insert({
                ...
                answer_explanation: q.answer_explanation // Mapping new field
            })
            .select()
            .single();
        ...
        // 2. Insert Options (for MCQ/TrueFalse)
        if (q.options && q.options.length > 0) {
            const options = q.options.map(opt => ({
                ...
                explanation: opt.explanation // Option-specific explanation
            }));
